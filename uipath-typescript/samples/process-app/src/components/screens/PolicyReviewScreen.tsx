...
export const PolicyReviewScreen: React.FC<PolicyReviewScreenProps> = ({ onNavigate }) => {
  const { currentCase, selectedPolicy, refreshCase, selectedDfRecord, dfRecords, completeStep, moveToNextStep } = useCase();
  ...
  // Query DF for policy data - try filtered query first, fall back to dfRecords or unfiltered fetch
  React.useEffect(() => {
    const loadPolicyDfData = async () => {
      ...
      // First try: use pre-loaded dfRecords from CaseContext (no API call)
      const fromContext = findMatch(dfRecords || []);
      if (fromContext) {
        console.debug('[PolicyReviewScreen] Using DF record from context');
      ...
      try {
        ...
        let res: any;
        try {
          res = await sdk.entities.getRecordsById(entityId as string, {
        ...
        // Fallback: load all records (no filter) and match client-side
        const allRes: any = await sdk.entities.getRecordsById(entityId as string, { pageSize: 200 });
        const allItems: any[] = Array.isArray(allRes) ? allRes : allRes?.items ?? allRes?.data ?? [];
        match = findMatch(allItems);
  ...
  const handleSendToBeneficiaryResearch = async () => {
    ...
    try {
      ...
      // Update Data Fabric with the three fields
      if (sdk && dfData) {
        try {
          const entityId = import.meta.env.VITE_UIPATH_ENTITY_ID;
          if (entityId && dfData.Id) {
            console.log('[PolicyReviewScreen] Current dfData record:', dfData);
            console.log('[PolicyReviewScreen] Record ID:', dfData.Id);
            console.log('[PolicyReviewScreen] Decision notes value:', decisionNotes);
            console.log('[PolicyReviewScreen] Cause of death approval:', causeOfDeathApproval);
            console.log('[PolicyReviewScreen] Policy eligibility decision:', analystDecision);
            
            const updateData = {
              Id: dfData.Id, // Required for update
              CauseofDeathApproval: causeOfDeathApproval || null,
              PolicyEligibilityDecision: analystDecision || null,
              PolicyReviewNotes: decisionNotes || null
            };
            
            console.log('[PolicyReviewScreen] Updating Data Fabric with:', updateData);
            console.log('[PolicyReviewScreen] Entity ID:', entityId);
            
            const result = await sdk.entities.updateById(entityId, [updateData]);
            
            console.log('[PolicyReviewScreen] Data Fabric update result:', result);
            console.log('[PolicyReviewScreen] Data Fabric updated successfully');
          } else {
            console.warn('[PolicyReviewScreen] Cannot update DF - missing entityId or dfData.Id:', { entityId, dfDataId: dfData?.Id });
          }
        } catch (dfError) {
          console.error('[PolicyReviewScreen] Failed to update Data Fabric:', dfError);
          console.error('[PolicyReviewScreen] Error details:', JSON.stringify(dfError, null, 2));
          toast({ title: 'Warning', description: 'Policy updated but Data Fabric sync failed.', variant: 'destructive' });
        }
      } else {
        console.warn('[PolicyReviewScreen] Skipping DF update - sdk:', !!sdk, 'dfData:', !!dfData);
      }
        ...
        <div className="col-span-12 lg:col-span-5 bg-white rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="text-base">{(dfData || selectedDfRecord)?.DecedentName || (dfData || selectedDfRecord)?.decedentName || currentCase.decedentName}</div>
            </div>
            <div>
        ...
        <div className="col-span-12 lg:col-span-7 bg-white rounded-lg shadow p-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <div className="text-base font-mono">{(dfData || selectedDfRecord)?.PolicyNumber || (dfData || selectedDfRecord)?.policyNumber || '-'}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="text-base">{(dfData || selectedDfRecord)?.FNODDate || '-'}</div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="flex items-center justify-end gap-3 mt-6 pt-6 border-t">
          ...
          <button 
            ...
            disabled={isLoading}
          >
            Complete & Continue to Beneficiary Research
          </button>
        </div>
      </div>
    </div>
