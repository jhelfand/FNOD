...
export const SummaryScreen: React.FC<SummaryScreenProps> = ({ onNavigate, includeAgent = true }) => {
  const { currentCase, policies, selectPolicy, selectedDfRecord, selectedCaseInstanceId, completeStep, moveToNextStep } = useCase();
  const { sdk } = useAuth();
  const { caseFromDF, loading: dfLoading, error: dfError, getFieldSource, placeholderNote } = useDataFabricCase(
    currentCase?.id,
  ...
  useEffect(() => {
    // If a DF record is selected, prioritize its data over entity lookup
    if (selectedDfRecord) {
      console.debug('[SummaryScreen] Selected DF Record:', selectedDfRecord);
      console.debug('[SummaryScreen] All DF Record keys:', Object.keys(selectedDfRecord));
      
      const getField = (obj: any, keys: string[]) => {
        for (const k of keys) {
          if (obj[k] !== undefined && obj[k] !== null) {
            console.debug(`[SummaryScreen] Found field ${k}:`, obj[k]);
            return obj[k];
          }
        }
        return null;
      ...
      setDfCaseId(selectedDfRecord.Id ?? selectedDfRecord.id ?? selectedDfRecord.CaseNumber ?? selectedDfRecord.caseId ?? '#placeholder');
      setDfCallerName(getField(selectedDfRecord, ['FNODCaller', 'CallerName', 'Caller', 'callerName']) ?? '#placeholder');
      setDfCallerPhone(getField(selectedDfRecord, ['phonenumber', 'PhoneNumber', 'FNODPhone', 'CallerPhone', 'Caller_Phone', 'Phone', 'callerPhone']) ?? '#placeholder');
      setDfPolicyNumber(getField(selectedDfRecord, ['PolicyNumber', 'policyNumber', 'Policy_No', 'Policy']) ?? '#placeholder');
      setDfPolicyType(getField(selectedDfRecord, ['PolicyType', 'ProductType', 'productType']) ?? '#placeholder');
      
      const faceValue = getField(selectedDfRecord, ['PolicyValues', 'policyvalue1', 'PolicyValue', 'FaceValue', 'faceValue', 'Amount']);
      console.debug('[SummaryScreen] Face Value found:', faceValue);
      setDfPolicyValue(faceValue ? Number(faceValue) : null);
      return;
    }

    // Fallback to entity lookup if no DF record selected
    let mounted = true;
    const loadFromEntity = async () => {
      try {
        const entityId = import.meta.env.VITE_UIPATH_ENTITY_ID;
        if (!entityId || !sdk) return;
        const res: any = await sdk.entities.getRecordsById(entityId as string, { pageSize: 1 });
        let items: any[] = [];
        ...
        if (mounted && first) {
          setDfCaseId(first.Id ?? first.id ?? null);
          ...
          const callerName = getField(first, ['FNODCaller', 'CallerName', 'Caller', 'callerName']);
          const callerPhone = getField(first, ['phonenumber', 'PhoneNumber', 'FNODPhone', 'CallerPhone', 'Caller_Phone', 'Phone', 'callerPhone']);
          const policyNumber = getField(first, ['PolicyNumber', 'policyNumber', 'Policy_No', 'Policy']);
          const policyType = getField(first, ['PolicyType', 'ProductType', 'productType']);
          const faceValue = getField(first, ['PolicyValues', 'policyvalue1', 'PolicyValue', 'FaceValue', 'faceValue', 'Amount']);

          setDfCallerName(callerName ? String(callerName) : null);
          setDfCallerPhone(callerPhone ? String(callerPhone) : null);
          setDfPolicyNumber(policyNumber ? String(policyNumber) : null);
          setDfPolicyType(policyType ? String(policyType) : null);
          setDfPolicyValue(faceValue ? Number(faceValue) : null);
        }
      } catch (e) {
        console.debug('Failed to load Data Fabric record for case id', e);
      }
    };

    loadFromEntity();
    return () => { mounted = false; };
  }, [sdk, selectedDfRecord]);
