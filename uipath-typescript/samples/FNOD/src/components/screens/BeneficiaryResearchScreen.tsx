...
export function BeneficiaryResearchScreen({ onNavigate }: BeneficiaryResearchScreenProps) {
  const { currentCase, selectedPolicy, selectedDfRecord, completeStep, moveToNextStep, refreshDfRecords } = useCase();
  ...
  const handleCompleteBeneficiaryResearch = async () => {
    ...
    try {
      ...
      if (sdk && entityId && dfRecord?.Id) {
        try {
          ...
          toast({ title: 'Warning', description: 'Beneficiary research complete but Data Fabric sync failed.', variant: 'destructive' });
        }
      }
      ...
      toast({ title: 'Error', description: 'Failed to complete beneficiary research.', variant: 'destructive' });
    } finally {
...
We are writing to you regarding the life insurance policy for ${selectedDfRecord?.DecedentName ?? selectedDfRecord?.decedentName ?? currentCase?.decedentName ?? '[Decedent Name]'}.
    ...
    <div className="space-y-6 animate-slide-up">
      ...
      <div className="space-y-4">
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          ...
          >
            Review Beneficiary Document
          </Button>
          <Button
            size="lg"
            onClick={handleCompleteBeneficiaryResearch}
        ...
        {/* PDF Viewer Modal */}
        <Modal open={isPdfViewerOpen} onClose={() => setIsPdfViewerOpen(false)} title="Beneficiary Document">
          <div className="w-full h-[70vh] min-h-[400px]">
            <iframe
              src="/Policy70008000.pdf"
              className="w-full h-full border-0 rounded"
              title="Beneficiary Document PDF"
            />
          </div>
        </Modal>
