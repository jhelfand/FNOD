...
export default function PayoutCalculatorScreen({ onNavigate }: PayoutCalculatorScreenProps) {
  const { currentCase, selectedPolicy, selectedDfRecord, completeStep, completeAllSteps, setCaseStatus, setCurrentStep, setPolicyStatus, refreshDfRecords } = useCase();
  ...
  useEffect(() => {
    const loadPolicyDfData = async () => {
      ...
      try {
        ...
        if (filtered.length > 0) setDfData(filtered);
      } catch (error) {
    ...
    try {
      ...
      if (sdk && entityId && dfRecordToUpdate?.Id) {
        try {
          ...
          toast({ title: 'Warning', description: 'Payout submitted but Data Fabric sync failed.', variant: 'destructive' });
        }
      }
        ...
        <Card className="col-span-12 lg:col-span-3 bg-white border rounded-lg">
          <CardContent className="pt-4 pb-4">
            <div>
              ...
              <p className="font-medium">{(dfForCaseFields as any)?.DecedentName ?? (dfForCaseFields as any)?.decedentName ?? (dfRecord as any)?.DecedentName ?? (dfRecord as any)?.decedentName ?? currentCase.decedentName}</p>
        ...
        <Card className="col-span-12 lg:col-span-3 bg-white border rounded-lg">
          <CardContent className="pt-4 pb-4">
            <div>
              ...
              <p className="font-medium font-mono">{(selectedDfRecord?.PolicyNumber ?? (Array.isArray(dfData) && dfData[0]?.PolicyNumber)) ? `TL-${selectedDfRecord?.PolicyNumber ?? dfData[0]?.PolicyNumber}` : '-'}</p>
        ...
        <CardContent>
          <div className="grid grid-cols-12 gap-x-6 gap-y-4 items-start">
            ...
            <div className="col-span-12 lg:col-span-3">
              ...
              <p className="font-medium">{selectedDfRecord?.FNODDate ?? (Array.isArray(dfData) && dfData[0]?.FNODDate) ?? '-'}</p>
            </div>
            <div className="col-span-12 lg:col-span-3">
